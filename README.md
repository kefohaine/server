...
| www.fxmq.net      | Cloudflare (proxied) → VPS IP | Anyone on the internet             | Serves an empty `index.html` at `/`; Caddy file browser at `/download` over the `download/` drop folder; two showcase pages at `/1` `/2` (maximalist vhost indexes in `services/fxmq.net/www/`) |
