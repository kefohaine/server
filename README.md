...
One VPS, one host. Cloudflare fronts the seven public hostnames (proxied — cloud, vault, kuma, www — DNS-only: mail, mc, talk); the Tailscale-only hostname is invisible on the public internet. Deliberate non-public behaviours (`tail.fxmq.net` being unreachable off the tailnet, Cloudflare Bot Fight Mode blocking `curl`/desktop sync against `cloud.fxmq.net`) are documented as rationale in `docs/GUIDE.md` so future agents don't "correct" them.
...
| mc.fxmq.net       | Cloudflare DNS-only → VPS IP | Anyone on the internet             | PufferPanel at `/panel`; in-browser Minecraft 1.8.8 (EaglercraftX, no install/login) at `/play` with websocket at `/play/server` (Java players: `mc.fxmq.net:25565`, any Java client 1.7.10–latest via the Via family); `/` redirects to `/panel`; unknown paths answer `ok`. DNS-only (grey cloud) so the game ports bypass the CF proxy |
| www.fxmq.net      | Cloudflare (proxied) → VPS IP | Anyone on the internet             | Landing stub answering `ok` on `/` (homepage stays a stub by design); Caddy file browser at `/download` over the `download/` drop folder; three showcase pages at `/1` `/2` `/3` (maximalist vhost indexes in `services/fxmq.net/www/`) |
...
The trade-off: browser traffic is bot-challenged. For terminal `curl` or Nextcloud desktop sync clients that can't solve Cloudflare's Browser Integrity Check, the workaround is a per-hostname WAF rule skip on the affected hostname (`cloud.fxmq.net`) — see the rationale in `docs/GUIDE.md`.

### Why Tailscale for tail.fxmq.net
...
- The `tail.fxmq.net` vhost in Caddy uses a LE cert just like every other vhost — tailnet clients get a clean HTTPS handshake instead of a self-signed cert warning.
...
- The Tailscale split DNS on the user side forwards queries for `tail.fxmq.net` to a resolver on the VPS.
- That resolver must return `100.117.144.0` for `tail.fxmq.net` and forward everything else.
