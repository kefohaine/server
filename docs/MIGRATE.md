...
3. Gotchas that must hold on the new host: the panel template writes `server.properties` at install from its `ip`/`port` data fields (the live servers bind `server-ip=0.0.0.0` / `server-port=25565|25567`); RCON disabled; the game domain's CF A record is DNS-only.
