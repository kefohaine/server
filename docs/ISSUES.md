...
- **`tail.$DOMAIN` mini terminal + gated shell** — unauthenticated command terminal at `/` (`go <vhost> [page]` / `log in` / `log out`, live prediction) and `basic_auth` (user `kefohaine`) on `/__tail/login` + `/ttyd`; the browser's cached credential is the session; `make tail-auth` sets/rotates it and prints the password once.
