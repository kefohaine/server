...
#### Storage VPS onboarded (Setup A) — NFS live datadirectory, DB stays on fxmq
...
- **Residual**: the first live run (2026-09-10) failed at the mount because the NC host had no `nfs-common` (no `mount.nfs` helper) — fixed in `scripts/storage.sh`; the datadirectory is still local under `cloud/users` (intact). Re-run `make storage` from the NC host to complete the move. The 2026-09-01 reset (fresh install, recovery manifests) stands as the current working state.
