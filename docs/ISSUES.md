...
- **Makefile `restart-<ctn>` only restarted one container in multi-container compose** — `restart-mc` restarted `mc` only, leaving `mc-web` untouched (and any dashboard template change required a separate `docker compose restart mc-web`). Fixed by restarting every container in the compose file. `logs-mc` now tails both with `[container]` prefixes.
