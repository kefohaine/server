...
- **Problem**: Any tailnet device can reach `server.jehpok.com` with no authentication. DNS-obscurity is the only access control — the link shortener admin UI at `/share/dashboard` and the host ttyd shell at `/shell` (a host systemd unit running as `debian` with full host control) both sit behind `@not_tailnet` and nothing else.
- **Fix**: Add Caddy `basic_auth` on the `/share/dashboard*` and `/shell*` matchers (needs a username + bcrypt hash from the operator), or apply Tailscale ACLs in the admin console to restrict who can reach the VPS at all.
...
### TLS validation on `server.jehpok.com` requires trusting the Cloudflare Origin CA
Looks like: a tailnet device hits `https://server.jehpok.com/...` and the browser/curl rejects the certificate (`unknown CA`, `SSL certificate problem: unable to get local issuer certificate`). Reality: `server.jehpok.com` is not in Cloudflare DNS, so tailnet devices connect directly to Caddy on the VPS — Caddy serves the Cloudflare Origin cert from `$(REPO)/certs/cert.pem`, signed by `CloudFlare Origin SSL Certificate Authority`, which is **not** in the system trust store. Cloudflare-fronted hostnames (www/share/api/vault/cloud/kuma) don't hit this because the client → Cloudflare edge handshake uses a public cert, not the Origin cert. Fix per tailnet device: download `https://developers.cloudflare.com/ssl/static/authenticated_origin_pull_ca.pem` and install it as a trusted CA (Debian/Ubuntu: copy to `/usr/local/share/ca-certificates/` then `sudo update-ca-certificates`; macOS: Keychain → System → Certificates → drag the .pem in and mark Always Trust; Firefox: Settings → Privacy & Security → Certificates → View Certificates → Authorities → Import). **A failed TLS validation on `server.jehpok.com` is the Origin cert working as designed** — install the CA once per device and the routes load normally.
...
Looks like: the link shortener admin UI is unreachable from `share.jehpok.com/share/dashboard`. Reality: the public vhost hides `/share`, `/api/*`, and `/healthz` via a Caddy `@admin` matcher returning 404, because admin has no app-level auth and the public side shouldn't leak its existence. Admin lives at `server.jehpok.com/share/dashboard`, which is tailnet-only.
...
- **Minecraft server** — Paper + Geyser/Floodgate at `mc.jehpok.com` (Java `:25565`, Bedrock `:19132`); tailnet-only Flask+rcon dashboard at `server.jehpok.com/mc/dashboard` (start/stop/restart, live stats, editable `server.properties`, log tail, quick rcon). World data bind-mounted under `$(REPO)/minecraft/data`.
