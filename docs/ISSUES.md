...
- **lazymc v0.2.11 (latest release, Mar 2024) is Java-only and its status decoder (minecraft_protocol v1_20_3 crate) fails on Paper 26.2 status responses in two ways** — it errors on a missing `favicon` (Paper omits it without a `server-icon.png`) and on descriptions with `extra` (any colored MOTD); either failure means lazymc never detects the server online. Fixes that worked: 64×64 `server-icon.png` in the server dir + a plain-text MOTD (no `§` codes).
