...
- **Progress 2026-09-06**: contributors API converged (kefohaine 666) and attribution is confirmed (noreply id matches, `author.login` = kefohaine), but the profile graph is still all `level=0` from 2026-08-01 on — no fresh push since the rewrite, so the graph aggregate never got a recompute pass; this commit is the nudge.
