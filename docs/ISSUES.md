...
- **`tail.$DOMAIN` now requires basic_auth** — Caddy `basic_auth` (user `op`) added behind the existing non-tailnet 403 gate; `make tail-auth` sets/rotates it and prints the password once (only the bcrypt hash is committed).
