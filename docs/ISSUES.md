...
- **`tail.$DOMAIN` dashboard + gated shell** — terminal dashboard at `/` (tailnet-only; vhost list + log in/out buttons) and Caddy `basic_auth` (user `kefohaine`) on `/ttyd`; `make tail-auth` sets/rotates the password and prints it once (only the bcrypt hash is committed).
