...
- **Makefile `restart-<svc>` only restarted one service in multi-service compose** — `restart-mc` restarted `mc` only, leaving `mc-web` untouched (and any dashboard template change required a separate `docker compose restart mc-web`). Fixed by restarting every service in the compose file. `logs-mc` now tails both with `[container]` prefixes.
...
- **Minecraft dashboard redesigned** — Aternos-style tabs (Status / Console / Players / Settings / Files / World), 2 s polling for live state (no page reloads), per-player actions (kick/ban/op/whitelist/gamemode) wired through rcon, per-player stats parsed from `/world/players/stats/<uuid>.json`, file browser with safe upload + Paper/Bukkit/Spigot YAML editor, world tar-stream download + zip upload + backup-and-regenerate flow. `make backup-mc` tars the world folder; `backup-all` chains it; `daily.sh` stops the game container before `backup-all` and restarts it after, so the world tar is quiescent. All routes still under `/mc/`.
