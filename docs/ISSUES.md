...
- **Problem**: `make bkp-all` (daily timer) tars Nextcloud `/data` in maintenance mode, but there is no SQLite-consistent DB snapshot (`sqlite3 .backup`) and no off-site copy target.
- **Fix**: Add `scripts/backup.sh`: `occ maintenance:mode --on` → DB snapshot (`sqlite3 .backup` now, `mariabackup` post-migration) + `rsync` of `cloud/users` → `--off`. Chain into `daily.sh` or cron it.
...
- **A panel container restart while the game server runs gracefully stops and removes it** — the daemon reconciles at boot; `make update` → `d-recreate-all` (daily) therefore kicks players and puts the game back to sleep (see `Open`).
