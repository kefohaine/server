...
- **Custom Caddy image is rebuilt by `make recreate-domain`** — `services/domain/Dockerfile` builds `caddy-dns:local` via xcaddy (Go 1.25 + commit `a8737d0` of caddy-dns/cloudflare, which relaxed the API token regex from `{35,50}` to accept the new 53-char CF token format).
