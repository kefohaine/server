...
- After any debug hell — append a lesson to `docs/GUIDE.md` (safety rule 11). The entries are the most valuable future-agent artifact; don't skip them.
...
2. **If this task was a debug hell** (symptom didn't match the obvious cause, took multiple non-obvious hops to root-cause, or surfaced a non-documented tool/config interaction), append a lesson to the lessons area in `docs/GUIDE.md` per Safety rule 11. Do this *before* the doc audit, not after — the lesson is the most valuable future-agent artifact.
