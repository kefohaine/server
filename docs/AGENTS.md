...
10. **Categorize `Solved` history by month.** In `docs/ISSUES.md`, group resolved items under `### Mon YYYY — short label` headings (e.g. `### Jul 2026 — early system build`). Add a new heading when the month changes, not per item. **One line per resolved issue** — format: `- **Short title** — one-sentence summary of what was done.` Never multi-line entries, sub-bullets, or paragraphs in the Solved section. The full root-cause / symptom / fix narrative belongs in an open issue entry while it's being worked; once resolved, collapse it to one line.
