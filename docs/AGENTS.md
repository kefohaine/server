...
8. **Never read secrets, credentials, or sensitive user data.** Do not read, print, or inspect: database files (`*.db`, `links.db`, `owncloud.db`), Vaultwarden data (`/var/www/github/jehpok.com/vault/data/`), Nextcloud user files (`cloud/users/`), uploaded files (`share/files/`), `.env` files, SSH private keys, TLS cert/key files, Tailscale state, or any file containing credentials, tokens, passwords, or personal data. Reading these is a privacy violation — treat them as untouchable. If a task requires knowing a value from a secret file, ask the operator to provide it instead.
...
- Repo mounted read-only at `/srv`; static files served from `/srv/content/domain/{www}`. `/var/www/github/jehpok.com/share/files` is bind-mounted read-only into `domain` at `/files` and read-write into `share` at `/data/files` — served as `share.jehpok.com/files` (Caddy browse) and written to by the upload route. Nextcloud html root mounted read-only at `/nextcloud` for static fallback.
- Vhosts: `www` → static fileserver; `vps` → responds `ok` (Tailscale-only, with `/share*` reverse proxy to the shortener admin); `share` → URL shortener + file sharing; `vault` → reverse_proxy to `vault:80` (Vaultwarden); `api` → responds `ok` (no backend, reserved); `cloud` → `php_fastcgi cloud:9000` (dial 10s, read/write 300s, aligned to PHP-FPM's 200s terminate timeout), blocks internal paths (`/data/*`, `/config/*`, `/lib/*`, `/3rdparty/*`, `/templates/*`, `/occ`, `/console.php`, `/db/*`, `/updater/*`), redirects carddav/caldav to `/remote.php/dav`, 100m body max (matches Cloudflare free-tier cap), zstd/gzip. The `vps` vhost enforces Tailscale-only at the edge: a `@not_tailnet` matcher (`not remote_ip 100.64.0.0/10`) returns 403 for any non-tailnet source IP, on top of the DNS split.
