...
10. **Categorize `Solved` history by month.** In `docs/ISSUES.md`, group resolved items under `### Mon YYYY — short label` headings (e.g. `### Jul 2026 — early system build`). Add a new heading only when the month changes — never create a second heading for the same month; append to the existing one. **One line per resolved issue** — format: `- **Short title** — one-sentence summary.` Keep it as short as the existing entries (e.g. `**SSH hardened** — password auth + root login disabled, \`AllowUsers debian\`.`). Never multi-line entries, sub-bullets, or paragraphs. The full root-cause / symptom / fix narrative belongs in an open issue entry while it's being worked; once resolved, collapse it to one line.
