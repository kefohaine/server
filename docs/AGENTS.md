...
- The reload sequence is `make d-recreate-<ctn>` (or `make d-recreate-fxmq.net` for a single proxy edit; broken on this host — see ISSUES.md, use the `docker build` fallback); the post-reload smoke test must include: shell tailnet routes (source IP in `100.64.0.0/10`, e.g. `curl --interface <tailscale-ip>`) — `/` → 200 plain `ok`, `/ttyd` → ttyd, `/owui` → 200 `ok` (not proxied since 2026-08-28) — 403 from non-tailnet; every other vhost (`cloud`, `kuma`, `mail`, `mc`, `vault`, `www`) → 200 plain `ok` from any source, `Issuer: Let's Encrypt` on a fresh TLS handshake.
...
- The Minecraft server (PufferPanel-managed Paper server `2ecfbe8c`; server config at `/var/www/custom/projects/homelab/puffer/data/servers/2ecfbe8c.json`, container created by the PufferPanel daemon) is started/stopped **only** from the PufferPanel UI/API (`mc.fxmq.net/panel`) — no auto start/stop, no sleep/wake stack (lazymc + mc-idle-sleeper removed 2026-08-28; see `docs/GUIDE.md`). Manual start/stop/restart, and edits to its config / launch args / `-Xmx` / container limits, remain forbidden without explicit operator approval. Read-only inspection (`jcmd`, `jstat`, `docker stats`, `docker inspect`, reading the server JSON) is always allowed. **Note (2026-08-28): the `mc.fxmq.net` vhost now answers plain `ok`, so the panel is unreachable through the vhost until its routes in `services/fxmq.net/vhosts/mc.fxmq.net.caddy` are restored — the game server keeps running as-is meanwhile.**
