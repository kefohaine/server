...
15. **Never read image files.** Never open, view, or otherwise inspect the content of image files (jpg/png/webp/gif/etc.) — no image viewers, thumbnails, or OCR. Image content is never needed for repo work and can carry personal data; if a task depends on what an image shows, ask the operator to describe it instead.
