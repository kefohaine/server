...
- The reload sequence is `make d-recreate-<ctn>` (or `make d-recreate-fxmq.net` for a single proxy edit; broken on this host — see ISSUES.md, use the `docker build` fallback); the post-reload smoke test must include: shell tailnet routes — `/` → 200 plain `ok`, `/ttyd` → ttyd — 403 from non-tailnet, public vhosts 200/302 with `Issuer: Let's Encrypt`. `mc.fxmq.net` routes: `/` → 308 to `/panel`, `/panel` → panel (200/302), `/download` → file listing, `/panel/register` + `/auth/register` → 403.
