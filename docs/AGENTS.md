...
- The Minecraft game server (`mc`) and its dashboard (`mc-web`) live in **separate compose files** — `services/mc/docker-compose.yml` (game) and `services/mc/docker-compose.mc-web.yml` (dashboard). They can be recreated independently (`make recreate-mc` vs. `make recreate-mc-web`); a dashboard-only change never restarts the game.
