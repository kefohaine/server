...
The wildcard Cloudflare Origin cert at `$(REPO)/certs/cert.pem` serves the `server.jehpok.com` vhost because Caddy picks one cert per listener and the wildcard matches. Tailnet devices see an untrusted-CA warning when they connect directly (since `server.jehpok.com` is not in Cloudflare DNS). The canonical explanation lives under "Intended" in `docs/ISSUES.md` — read that before considering any cert change. **Do not introduce `tls internal`, `tls off`, a separate listener, or a per-host cert mount** — each fails or breaks the other six vhosts.
