...
- The reload sequence is `make d-recreate-<ctn>` (or `make d-recreate-fxmq.net` for a single proxy edit; broken on this host — see ISSUES.md, use the `docker build` fallback); the post-reload smoke test must include: shell tailnet routes — `/` → 200 plain `ok`, `/ttyd` → ttyd, `/owui` → Open WebUI chat (`/auth` → Open WebUI login) — 403 from non-tailnet, public vhosts 200/302 with `Issuer: Let's Encrypt`. `mc.fxmq.net` routes: `/` → homepage (contains `mc.fxmq.net`), `/status` → JSON, `/panel` → panel (200/302), `/download` → file listing, `/panel/register` + `/auth/register` → 403. The Open WebUI chat has host access equal to ttyd's (`op` via SSH, dedicated key) — the Host Shell tool's working directory defaults to the repo.
