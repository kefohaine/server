...
3a. **Commit messages are one line, always.** The `make git-com` / `git-all` recipes pass `MSG=$(MSG)` through bash `-c`, so any embedded newline breaks the `git commit -m "..."` quoting with `unexpected EOF` — and the make invocation fails before the commit lands. Use a single line for `make git-com MSG="…"` / `git-all MSG="…"`. If the change genuinely needs context that won't fit on one line, put it in the commit body via `git commit -F <file>` or the commit message conventions in `docs/ISSUES.md` "Solved" entries (one-line per item). The diff speaks for itself; the message just has to round-trip through bash.
...
This project (`jehpok.com`) is a self-hosted Debian VPS. The full project guide lives in `docs/GUIDE.md`. The protected host resources, the canonical `make` recipe names, and any per-project safety constraints are listed there — never assume what they are without reading `docs/GUIDE.md`.
