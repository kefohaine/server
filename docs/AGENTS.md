...
- **Browser Minecraft (public no-sign-up playground, operator-approved 2026-08-30)** — PufferPanel server `07fd7727` (Paper 1.12.2, Java 17 via `eclipse-temurin:17`, host-net port 25567, plugins EaglerXServer + SkinsRestorer) serves the official EaglercraftX 1.8.8 client at `mc.fxmq.net/play` (static single-file client in `homelab/eaglercraft/`, outside the repo; default server seeded via the client's `servers` hint (wss://mc.fxmq.net/server — no per-browser Add Server needed)), websocket multiplexed on the MC port (Caddy `/server` → `172.22.0.1:25567`), ViaVersion/ViaBackwards/ViaRewind bridging any Java client 1.7.10→latest to the Paper 1.12.2 backend; Chunky 1.1.21 (patched build) for pregen. Deliberately `online-mode=false` + no whitelist + `run.autostart: true` + `run.autorestart: false` (public playground — anyone joins as any name; don't "harden" it into online mode without operator approval; autorestart has been off since 2026-08-31 — the operator disabled it while pregen-testing the patched Chunky, so crashes no longer self-bounce and the server is started manually from the panel; do not "restore" autorestart without approval). UFW allows `25567/tcp` from anywhere (opened 2026-08-31 — Java clients join the playground directly) plus the docker bridge (172.22.0.0/16) rule, so the vanilla game port is public, not browser-only. The paperdl install step is guarded with `!file_exists('server.jar')` so a panel re-install can't clobber the working new-paperclip jar (see `docs/ISSUES.md` Lessons learned). Terralith / Distant Horizons are not available for this stack (Chunky was built from source for 1.12.2 — see `docs/ISSUES.md`).
