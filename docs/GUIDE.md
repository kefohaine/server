...
- **Browser Minecraft (EaglercraftX 1.8.8 — server `07fd7727`)**: public no-sign-up playground at `https://mc.fxmq.net/play` — the official **EaglercraftX 1.8 u53** client (single-file HTML from eaglercraft.com, in `homelab/eaglercraft/`, bind-mounted into Caddy at `/eaglercraft:ro`; the default server is SEEDED into every client via the `servers` hint in `eaglercraftXOptsHints` (patched into the client's index.html — u53 supports `{addr, name}` entries; without it each browser starts with an empty server list)). Backend: PufferPanel server `07fd7727` (deployable JSON `config/pufferpanel/servers/07fd7727.json`) — Paper 1.12.2 (new-paperclip jar, `git-Paper-1620`; fill.papermc.io 1205's old paperclip hits the dead Mojang S3 URL — Lessons learned), Java 17, host-net, `server-port=25567`, `online-mode=false`, no whitelist, Aikar flags, plugins **EaglerXServer** + **SkinsRestorer** + **ViaVersion** + **ViaBackwards** + **ViaRewind** (the Via family makes **any Java client from 1.7.10 to the latest** connect; the browser client is EaglercraftX 1.8.8 — translated by ViaVersion) + **Chunky** (1.1.21, rebuilt from source with the ≥1.13 version gate patched out — see ISSUES; pregen from the panel console: `chunky radius <n>` + `chunky shape circle` (shape is a separate subcommand in 1.1.21 — `start` ignores positional args, default shape is square) + `chunky start` — **type console commands WITHOUT the leading `/`**: this Paper 1.12.2 build never strips it on the console path, so any slash-prefixed console command (even vanilla `/help`) answers "Unknown command" — see ISSUES Lessons learned; Chunky 1.1.21's `start` takes no radius argument, the radius comes from `chunky radius`). **The deployed `plugins/Chunky.jar` also carries a patched `AsyncChunksPaper_9_12`** (main-thread, time-sliced chunk loading — Paper 1.12.2's async chunk-IO trips AsyncCatcher and a plain main-thread load trips the 60s watchdog; patch source + stock jar kept in `plugins/chunky-patch/`, see ISSUES Lessons learned). EaglerXServer multiplexes the websocket on the MC port — Caddy `/server` → `172.22.0.1:25567` (renamed from `/ws`→`/web`→`/server` 2026-08-31; older paths now 404). `run.autostart` true, `run.autorestart` **false since 2026-08-31** (operator disabled it while pregen-testing the patched Chunky — crashes no longer self-bounce; start manually from the panel; do not "restore" it without approval); paperdl step guarded with `!file_exists('server.jar')`. Tuning (2026-08-31): 4 GiB heap, `view-distance=4` set in server.properties but **overridden to 6 by spigot.yml `world-settings.default.view-distance`** (server logs "View Distance: 6"; operator decision pending — see ISSUES), eaglercraft render 4 (`eagler_players_view_distance: 4`), entity spawn-limits halved + activation-range halved. **Java players**: `mc.fxmq.net:25567` is PUBLIC (UFW `25567/tcp` anywhere — opened 2026-08-31) and accepts Java clients **1.8–1.12.2** (ViaVersion; 1.13+ needs ViaBackwards — not installed); offline-mode = any username, no auth. MOTD/skin branding in `plugins/EaglercraftXServer/listener.yml` + `settings.yml`. `make smoke` asserts `/play` 200 only — the `/server` websocket check was removed 2026-08-31 (operator: the game server isn't run 24/7, so a 502 while it's stopped is expected; see `scripts/smoke-vhosts.sh`).
...
Nextcloud talks to the signaling server over the bridge (`http://172.22.0.12:8080`, registered first so the server uses it) and tells clients about the public endpoint (`https://turn.fxmq.net/signaling`, registered second). The Caddy vhost strips the `/signaling` prefix before proxying — the signaling server serves its websocket at `/spreed` and its API at `/api/v1/*`.
...
- **Signaling registration in Nextcloud** — `occ talk:signaling:add` twice: the internal URL first (server-side connection), the public URL second (clients):
  `docker exec -u www-data nextcloud php occ talk:signaling:add http://172.22.0.12:8080 <SIGNALING_SECRET>`
  `docker exec -u www-data nextcloud php occ talk:signaling:add https://turn.fxmq.net/signaling <SIGNALING_SECRET>`
  The `[backend1] secret` in `server.conf` must match. The `[backend1] urls` is the Nextcloud instance the signaling server calls back into (`https://cloud.fxmq.net`).
