...
- **`kuma`** — image, bind mount, healthcheck: `services/kuma/docker-compose.yml`. Monitor definitions live in Kuma's SQLite, edited via the UI. HTTP monitors probe `https://172.22.0.1` with `hostname` set to the public vhost (SNI) — direct to Caddy on the host bridge, bypassing Cloudflare. Otherwise CF Bot Fight Mode returns 403 to the Kuma probe UA.
