...
- **Git identity** — all commits (this repo and `kefohaine/web-app`, history rewritten 2026-09-03) are authored `kefohaine <268671487+kefohaine@users.noreply.github.com>` (repo-local `git config`). The account-noreply form auto-attributes to the account (id 268671487) with **no verification step** — use it; a custom email (`[redacted]`, retired) counted nothing on the profile graph until verified.
- **History rewrites (lesson 2026-09-02)** — `git filter-branch` silently drops one side of this repo's merge DAG (651 → 520 commits, the jehpok-era line) even on a pristine clone, and `git filter-repo` drops it too (verified 2026-09-03: 659 → 528). This history has **two roots** (jehpok universe merged in via `Merge branch 'main' of …/friedutch/homelab`); never use either tool here. Rewrites use `scripts/identity-rebuild.sh` (email swap) or `scripts/drop-path.sh` (path purge, e.g. the 2026-09-03 `services/nextcloud/.env` removal) — plumbing, parents-first, multi-root-safe. Verify after every run: identities uniform, tree of tip unchanged, messages + dates byte-identical, commit count / 2 roots / 9 merges preserved, `git fsck` clean. **No backup tags exist** — snapshots of pre-rewrite histories were destroyed 2026-09-03 (they held the scrubbed old identities + the `.env` secret); the jehpok-era record survives only inside `main` under the noreply identity (operator's explicit choice). Never create remote tags of old histories: GitHub's Pulse/"all branches" and contributor aggregates count **every ref including tags** and map the old noreply emails to the account, so a pushed pre-rewrite tag re-exposes scrubbed identities ([redacted] 172 commits surfaced as a "170 op" contributor) and poisons the stats.
