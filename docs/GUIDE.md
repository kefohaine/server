...
make d-recreate-<ctn>  # force-recreate one container — d-recreate-fxmq.net (rebuilds) | d-recreate-nextcloud | d-recreate-vaultwarden | d-recreate-uptimekuma | d-recreate-pufferpanel | d-recreate-open-webui | d-recreate-mc-home
...
- `/var/www/custom/projects/homelab/open-webui/` — Open WebUI runtime dir: `data/` (sqlite with users/chats/tools, uploads), `keys/host_ed25519` (SSH identity for the Host Shell tool; public half in op's `authorized_keys`), and `openwebui.env` (DeepSeek key + `WEBUI_SECRET_KEY` + admin creds, chmod 0600). None of it is in the repo.
...
- `/var/www/custom/projects/homelab/mail/` — mail platform data: `data/` (Maildirs), `state/`, `logs/`, `config/` (DMS config incl. DKIM keys), `roundcube/` (webmail sqlite). Owned 5000:5000 / 33:33 as noted in the compose file.
...
SSH is restricted to the Tailscale network only — the public internet cannot reach port 22. Password auth is disabled; root login is key-only (`PermitRootLogin prohibit-password`); `AllowUsers op root`. Config: `/etc/ssh/sshd_config.d/50-cloud-init.conf`. The installer (`scripts/install.sh`) writes this file for user `op` and keeps root key access. The Open WebUI container reaches sshd from the docker bridge (UFW `172.22.0.0/16 → 22/tcp`) with a dedicated `restrict,no-pty` key in op's `authorized_keys` (see the `ai` service pointer).
...
| mc-home | `services/mc-home/docker-compose.yml` | `mc-home` | `make d-restart-mc-home` | `make d-recreate-mc-home` |
| ai      | `services/open-webui/docker-compose.yml` | `open-webui` | `make d-restart-open-webui` | `make d-recreate-open-webui` |
| mail    | `services/mail/docker-compose.yml` | `mail` + `roundcube` | `make d-restart-mail` | `make d-recreate-mail` |
...
- **`mc-home`** — Minecraft homepage + status at `mc.fxmq.net/`: landing page at `/` and `/status` JSON pings of the game server's public ports (`172.22.0.1:25565` TCP Java status protocol, `172.22.0.1:19132` UDP RakNet — the game server is host-net). Stdlib-only Python (`services/mc-home/app.py`), pinned `172.22.0.11:5000`. The `/download` route is Caddy's own `file_server browse` over `homelab/download` (bind-mounted into the caddy container at `/download:ro`). **`mc.fxmq.net` A record MUST be DNS-only at Cloudflare** (grey cloud): CF's proxy only carries 80/443, so a proxied record breaks the game ports. Caddy's 443 still works DNS-only (DNS-01 cert). After any CF record change, `make restart-dnsmasq`.
...
- **`ai`** — Open WebUI at `shell.fxmq.net/owui` (Caddy `handle_path /owui` strips the prefix; the pinned build has no subpath support, so the app's absolute root routes `/api` `/static` `/ws` `/auth` are proxied at root too, and `/` itself responds `ok`) — see `services/open-webui/docker-compose.yml`. ChatGPT-style chat UI wired to DeepSeek (OpenAI-compatible; the connection + `WEBUI_SECRET_KEY` + `WEBUI_ADMIN_*` live in `/var/www/custom/projects/homelab/open-webui/openwebui.env`, outside the repo, chmod 0600 — same pattern as `caddy_data/CF_API_TOKEN`). Runtime data (sqlite: users/chats/tools, uploads) at `/var/www/custom/projects/homelab/open-webui/data`. Host access from the chat = the **Host Shell** tool (registered via the admin API into the sqlite `tool` table), which SSHes to `op@172.22.0.1` with the dedicated key `/var/www/custom/projects/homelab/open-webui/keys/host_ed25519` (mounted at `/keys:ro`; public half in op's `~/.ssh/authorized_keys` with `restrict,no-pty`). The container is on `net` at the pinned bridge IP `172.22.0.7:8080` (Caddy upstream). UFW allows `172.22.0.0/16 → 22/tcp` so the container can reach sshd. Admin creds: `WEBUI_ADMIN_EMAIL`/`WEBUI_ADMIN_PASSWORD` in the env file. DeepSeek models appear in the chat bar dropdown via the connection's `/v1/models` (flash / pro / vision-exp); image generation has no engine yet (see `docs/ISSUES.md` Open).
- **mail** — Docker Mailserver + Roundcube: `services/mail/docker-compose.yml` (mail at 172.22.0.9 publishes 25/465/587/993; roundcube at 172.22.0.10, webmail only). TLS is the per-vhost LE cert for mail.fxmq.net from caddy_data, mounted read-only (SSL_TYPE=manual; DMS changedetector reloads on renewal). Roundcube connects with STARTTLS over the bridge (dovecot/postfix reject plaintext auth) — the override file `services/mail/roundcube-tls.inc.php` + the compose `extra_hosts` entry map mail.fxmq.net to 172.22.0.9 so SNI/cert match. Mailboxes are managed with `docker exec mail setup email add|list|del <addr>`; credentials for admin@/postmaster@ are in `services/mail/.env` (gitignored). DKIM keys + generated DNS records live in `/var/www/custom/projects/homelab/mail/config/opendkim/keys/fxmq.net/`. DNS: MX + mail.fxmq.net A are DNS-only at Cloudflare; SPF/DMARC/DKIM TXT set (selector `mail`).
...
- **Dovecot/postfix reject plaintext SMTP/IMAP auth** — Roundcube must use STARTTLS (`tls://mail.fxmq.net:143` / `:587`). The env-derived `ROUNDCUBEMAIL_*` settings in the compose file cannot express the TLS prefix; the override lives in `services/mail/roundcube-tls.inc.php` (included by the image entrypoint) and the compose `extra_hosts` maps the FQDN to the bridge IP.
...
- **Inbound port 25 + PTR are provider-side** — the VPS provider filters inbound TCP 25 and the IP has no PTR, so external mail receipt/delivery reputation is blocked until the operator unblocks 25 and sets reverse DNS (see `docs/ISSUES.md`).
