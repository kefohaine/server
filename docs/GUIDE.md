...
- **`panel`** — PufferPanel game panel container: `services/pufferpanel/docker-compose.yml` (web 8080 on the bridge, SFTP `:5657`, mounts the Docker socket rw — required to start). Admin credentials: `/var/www/custom/projects/homelab/puffer/admin-pass.txt` (created at setup). Its vhost routes on `mc.fxmq.net` were retired 2026-08-28 (the vhost answers plain `ok`) — restore the routes in `services/fxmq.net/vhosts/mc.fxmq.net.caddy` to reach the panel at `/panel` again.
- **`mc.fxmq.net`** — decommissioned stub since 2026-08-28: every path answers plain `ok`. Previously served PufferPanel at `/panel` and a Caddy file browser at `/download` (the `homelab/download` drop folder is still bind-mounted at `/download:ro` but no longer routed). **`mc.fxmq.net` A record MUST be DNS-only at Cloudflare** (grey cloud): CF's proxy only carries 80/443, so a proxied record breaks the game ports. Caddy's 443 still works DNS-only (DNS-01 cert). After any CF record change, `make restart-dnsmasq`.
