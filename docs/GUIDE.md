...
- **Signaling registration in Nextcloud** — `occ talk:signaling:add` twice: the internal URL first (server-side connection), the public URL second (clients):
  ...
  `docker exec -u www-data nextcloud php occ talk:signaling:add https://talk.fxmq.net/signaling <SIGNALING_SECRET>`
...
- **Ordering after a fresh deploy** — DNS record → `make dok-recreate-fxmq.net` (issues the talk cert) → `make talk-gen` → `make dok-recreate-nextcloud-db` → `make dok-recreate-nextcloud` → occ app/config setup → `make install-cron`.
