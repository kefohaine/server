...
- **Browser Minecraft (eaglercraft 1.12.2 — server `07fd7727`)**: public no-sign-up playground at `https://mc.fxmq.net/play` — a static eaglercraft 1.12.2 client in `homelab/eaglercraft/` (bind-mounted into Caddy at `/eaglercraft:ro`; the launcher `index.html` is patched with the default server `wss://mc.fxmq.net/ws`). Backend: PufferPanel server `07fd7727` (deployable JSON `config/pufferpanel/servers/07fd7727.json`; drop into `puffer/data/servers/` + restart the panel) — Paper 1.12.2 (the new-paperclip jar from the Eaglercraft-Server-Paper template, `git-Paper-1620`; the fill.papermc.io build 1205 paperclip still hits the dead Mojang S3 URL — see Lessons learned), Java 17 (`eclipse-temurin:17`; EaglerXServer v1.1.1 requires it), host-net, `server-port=25567`, `online-mode=false`, no whitelist, Aikar JVM flags, plugins **EaglerXServer** + **SkinsRestorer**. EaglerXServer multiplexes the websocket on the MC port itself (Netty channel injection) — Caddy's `/ws` proxies to `172.22.0.1:25567`, and the plugin's 404 "Websocket Upgrade Failure" page on plain HTTP GETs is normal. `run.autostart: true` + `run.autorestart: true` are deliberate (playground auto-starts with the panel and self-heals on crash — unlike the protected 26.2 server); the `paperdl` install step carries a `!file_exists('server.jar')` guard so a re-install doesn't clobber the working jar. Tuning (operator-set 2026-08-31): 4 GiB heap (`-Xms4096M` + Aikar flags), `view-distance=4` (1.12.2 has no separate simulation-distance — view-distance drives ticking), eaglercraft players capped at render distance 4 (`eagler_players_view_distance: 4`), entity spawn-limits halved (`bukkit.yml`: 35/7/2/7) + entity-activation-range halved (`spigot.yml`: 16/16/8/8). UFW allows `25567/tcp` **only** from `172.22.0.0/16` (Caddy→host) — the vanilla game port is not public. MOTD/skin branding lives in `plugins/EaglercraftXServer/listener.yml` + `settings.yml` (server dir). `make smoke` asserts `/play` 200 (client page) + `/ws` 101 (websocket upgrade).
