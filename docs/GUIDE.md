...
- **Git identity** — all commits (this repo and `kefohaine/web-app`, history rewritten 2026-09-03) are authored `kefohaine <268671487+kefohaine@users.noreply.github.com>` (repo-local `git config`). The account-noreply form auto-attributes to the account (id 268671487) with **no verification step** — use it; a custom email (`[redacted]`, retired) counted nothing on the profile graph until verified. Pre-noreply state is on remote tag `history-developer-email-20260903`.
- **History rewrites (lesson 2026-09-02)** — `git filter-branch` silently drops one side of this repo's merge DAG (651 → 520 commits, the jehpok-era line) even on a pristine clone, and `git filter-repo` drops it too (verified 2026-09-03: 659 → 528). This history has **two roots** (jehpok universe merged in via `Merge branch 'main' of …/friedutch/homelab`); never use either tool here. Rewrites use `scripts/identity-rebuild.sh` (plumbing: `git hash-object -t commit`, parents-first, multi-root-safe) — verify after every run: authors/committers uniform, tree of tip unchanged, messages + dates byte-identical, `2 roots / 9 merges / 659 commits` preserved, `git fsck` clean.
- **Backup tags stay LOCAL-ONLY (lesson 2026-09-03)** — never push history-backup tags to the GitHub remote: GitHub's Pulse/"all branches" and contributor aggregates count **every ref including tags** and map the old noreply emails to the account, so a pushed pre-rewrite tag re-exposes the scrubbed old identities ([redacted] 172 commits surfaced as a "170 op" contributor; totals inflated 660 → 2389) and poisons the stats. Pre-rewrite snapshots (`history-backup-20260902`, `history-20260902-noreply`, `history-developer-email-20260903`) exist only in the local clone; re-push with `git push homelab <tag>` only if the local copy is ever at risk.
- **Post-rewrite GitHub web index can break (lesson 2026-09-03)** — after a full-history force-push, GitHub's web git-data index can 500/404 while git stays healthy: `github.com/kefohaine/server` root, `/tree/main` and `/commits` show "Page not found", and `api.github.com/repos/kefohaine/server/commits` + `/git/ref/heads/main` return HTTP 500 — yet `git ls-remote` (SSH + HTTPS), `raw.githubusercontent.com`, `codeload` zips and the `issues`/`pulse`/`releases` tabs all work (A/B: `web-app` returns 200 everywhere). Not a takedown and no data loss (local main == remote main). **Fixed 2026-09-03 without support**: pushing a new (empty) commit to `main` (`daf119d`) triggered the rebuild — expect 10-20 min of intermittent 500s/404s while it settles, then poll until 10/10 requests return 200 with a non-empty body (`curl -s "https://api.github.com/repos/kefohaine/server/commits?per_page=1" | wc -c`). Only contact GitHub support if the nudge doesn't take. **Guard**: `make gh-web-health` (scripts/gh-web-health.sh) checks the web git-data endpoints after any rewrite; the pre-push hook warns whenever a push replaces remote history; the nudge remedy is `git commit --allow-empty -m 'chore: nudge GitHub web git-data rebuild'` + push. **Attribution vs graph**: after a rewrite, commit attribution (API `author.login`, commit-page avatar, `/contributors` API) recovers quickly, but the profile contribution graph + repo "Contributors" sidebar widget lag — they're background aggregates. The repo contributors page can even show ghosts (a "claude" author with 47 commits that exists in **no** local or remote history — stale aggregate). 2026-09-03 fix: both repos' histories rewritten to the account-noreply email (auto-attributes unconditionally), so the graph depends on nothing operator-side — only on GitHub's recompute (check `https://github.com/users/kefohaine/contributions` for non-zero `data-level` cells; `docs/ISSUES.md` tracks it). Don't re-rewrite history to chase the graph — noreply commits are guaranteed-attributed; re-rewriting just risks re-triggering the index break.
...
- **The edge guardrail** — app vhosts (cloud/vault/kuma/mail) must serve their real app: the pre-commit hook (installed via `make install-hooks`) blocks a bare `respond "ok"` in them (mc/www/tail are exempt by design — mc's catch-all is `ok`, www serves static pages (empty `/` + `/download` + the `/1` `/2` showcases), tail serves the tailnet home) and runs `caddy validate` on staged `services/fxmq.net/` changes; the pre-push hook runs `make smoke`. Never bypass with `EDGE_OVERRIDE`/`SKIP_SMOKE` unless the operator explicitly asks. After any `docker restart fxmq.net` / `make dok-recreate-fxmq.net`, run `make smoke`.
