...
How to operate `homelab.com` — the project's repository layout, deployment commands, protected host resources, per-service edit-safe facts, and operational gotchas. Read this before editing anything.
...
- `config/` — reference copies of host-level configs (goose systemd unit, SSH hardening, dnsmasq, maintenance timer + script, Claude Code deny-list). Restored to live paths by `make install-config`. For an offline copy of the whole `config/` tree, use `make bundle-config`.
...
make recreate-all      # start/recreate all containers in order (share-flask, vhosts, nextcloud, vaultwarden, ut-kuma, homer, mc)
...
make recreate-<ctn>    # force-recreate one container — recreate-vhosts | recreate-nextcloud | recreate-share-flask | recreate-vaultwarden | recreate-ut-kuma | recreate-homer | recreate-mc
make d-restart-<ctn>   # restart every container in the compose file — after editing a mounted config; mc restarts the mc-server container, mc-flask restarts the dashboard
...
make d-logs-<ctn>      # follow one container's logs — d-logs-vhosts | d-logs-nextcloud | d-logs-share-flask | d-logs-vaultwarden | d-logs-ut-kuma | d-logs-homer | d-logs-mc
...
- `/var/www/custom/projects/homelab/certs/` — RETIRED. The wildcard Cloudflare Origin cert that used to live here is no longer referenced by any vhost (the per-vhost ACME certs serve every hostname). The directory and the files inside it are kept on disk for audit/history but can be deleted without breaking the running system. Prefer removing the mount + the files together so the new design is preserved.
- `/var/www/custom/projects/homelab/caddy_data/` — Caddy runtime data dir (LE certs under `caddy/acme/`, ACME account keys under `caddy/acme/`, OCSP staples under `caddy/certificates/`) and the `CF_API_TOKEN` file used for per-vhost ACME DNS-01. The Caddy container bind-mounts this dir at `/data` so certs persist across container recreates. The `CF_API_TOKEN` file is created by the operator (see `docs/MIGRATE.md`); do not commit it.
...
|----------|-----------------------------------------|-----------|------------------------------|------------------------------|
| domain   | `services/vhosts/docker-compose.yml`    | `vhosts`  | `make d-restart-vhosts`      | `make recreate-vhosts` (builds `caddy-dns:local` from `services/vhosts/Dockerfile`) |
| cloud    | `services/nextcloud/docker-compose.yml`     | `nextcloud` | `make d-restart-nextcloud`  | `make recreate-nextcloud`    |
| share    | `services/share-flask/docker-compose.yml`     | `share-flask` | `make d-restart-share-flask` | `make recreate-share-flask` |
| vault    | `services/vaultwarden/docker-compose.yml`     | `vaultwarden` | `make d-restart-vaultwarden` | `make recreate-vaultwarden` |
| kuma     | `services/ut-kuma/docker-compose.yml`      | `ut-kuma` | `make d-restart-ut-kuma`     | `make recreate-ut-kuma`      |
| homer    | `services/homer/docker-compose.yml`     | `homer`   | `make d-restart-homer`       | `make recreate-homer`        |
| mc-game | `services/mc/docker-compose.yml`        | `mc-server`               | `make d-restart-mc`         | `make recreate-mc`         |
| mc-web  | `services/mc/docker-compose.mc-flask.yml` | `mc-flask`              | `make d-restart-mc-flask`   | `make recreate-mc-flask` (builds `mc-flask` locally) |
...
- **`domain`** — vhosts, snippets, header policy: `services/vhosts/Caddyfile` (top-level config + per-vhost imports) and `services/vhosts/vhosts/*.caddy` (one per hostname). Compose + bind mounts + custom Dockerfile (Caddy built with the caddy-dns/cloudflare plugin for ACME DNS-01): `services/vhosts/docker-compose.yml`. Runs `network_mode: host` — see note above.
...
- **`share`** — Flask routes: `content/share/app.py`. Compose + DB bind mount: `services/share-flask/docker-compose.yml`.
- **`vault`** — env, bind mount, admin token: `services/vaultwarden/docker-compose.yml`.
- **`kuma`** — image, bind mount, healthcheck: `services/ut-kuma/docker-compose.yml`. Monitor definitions live in Kuma's SQLite, edited via the UI on first run.
...
- A deliberate `systemctl stop dnsmasq` takes all tailnet-side `*.homelab.com` resolution down. Restart with `make restart-dnsmasq`.
