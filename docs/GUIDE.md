...
- **Post-rewrite GitHub web index can break (lesson 2026-09-03)** — after a full-history force-push, GitHub's web git-data index can 500/404 while git stays healthy: `github.com/kefohaine/server` root, `/tree/main` and `/commits` show "Page not found", and `api.github.com/repos/kefohaine/server/commits` + `/git/ref/heads/main` return HTTP 500 — yet `git ls-remote` (SSH + HTTPS), `raw.githubusercontent.com`, `codeload` zips and the `issues`/`pulse`/`releases` tabs all work (A/B: `web-app` returns 200 everywhere). Not a takedown and no data loss (local main == remote main). Fix: GitHub support ticket with the 500-vs-200 evidence, or trigger a reindex with a new (even empty) commit; verify recovery with `curl -s -o /dev/null -w '%{http_code}' https://api.github.com/repos/kefohaine/server/commits` → 200.
