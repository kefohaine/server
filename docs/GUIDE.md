...
- `/var/www/custom/projects/homelab/pgdata/` — Nextcloud PostgreSQL data dir (bind-mounted into the `nextcloud-db` container). This directory migrates to the operator's 1 TB VPS when it joins the tailnet (see "Nextcloud DB" below).
...
| cloud   | `services/nextcloud/docker-compose.yml` | `nextcloud` + `redis` + `talk-signaling` + `talk-coturn` | `make dok-restart-nextcloud` | `make dok-recreate-nextcloud` |
| cloud-db | `services/nextcloud/docker-compose.db.yml` | `nextcloud-db` | `make dok-restart-nextcloud-db` | `make dok-recreate-nextcloud-db` |
...
1. **Signaling** (`talk-signaling`, the official Go server) — the "switchboard". Every Talk client keeps a websocket open to it (`wss://turn.fxmq.net/signaling/spreed`); the signaling server tells participants who else is in the call and relays small control messages. Nothing about the call *media* flows through it.
...
`services/nextcloud/docker-compose.yml` runs four containers: `nextcloud` (.5), `redis` (.11), `talk-signaling` (.12, strukturag/nextcloud-spreed-signaling — the official Go server, no NATS needed), `talk-coturn` (.14). RAM hard caps sum to 2.34 GB with the DB (1g + 192m + 256m + 128m + 768m) — the operator's 3 GB ceiling. All four carry compose healthchecks (`docker ps` shows them `healthy`); coturn's is a real STUN binding probe via the image's `turnutils_stunclient` — a bare TCP connect would pass while the UDP/STUN path is broken.
...
- **`make talk-gen` only creates missing secrets** — it never rotates existing ones. To rotate: delete the key from `services/nextcloud/.env`, re-run `make talk-gen`, re-run the `occ talk:signaling:add` / `talk:turn:add` commands with the new value, and restart `talk-signaling`/`talk-coturn`.
- **`talk-signaling` reads its config as uid 850** — `/var/www/custom/projects/homelab/talk/server.conf` must stay world-readable (644); `talk-gen` sets it. A 600 file makes the container fail at boot with a permission error.
