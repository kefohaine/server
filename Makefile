...
>@echo "  │  make install           :copy config/ to this server"
>@echo "  │  make update            :update [apt -> update/upgrade; docker -> pull/recreate]"
>@echo "  │  make backup            :backup [all container databases; server config to repo; secrets]"
>@echo "  │  make cleanup           :clean [apt -> autoremove/clean; docker -> prune builder/images/containers; backups -> keep latest 3 of each]"
>@echo "  └  make status            :global status list [git; systemd; docker; tmux; backups; mails]"
>@echo ""
>@echo "  GitHub Actions"
>@echo "  │  make git-pull         :pull remote changes"
>@echo "  │  make git-add          :stage changes"
>@echo "  │  make git-com MSG=     :commit staged changes"
...
>@echo "  │  make systemd-log      :tail target journal"
>@echo "  │  Variables"
