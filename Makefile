...
>@echo "  │  make nc-capture            │snapshot live NC users/groups/quotas into cloud/recovery/ outside the repo (recovery manifests — re-run after user changes)"
...
>@echo ""
>@echo "  Taildrop (send files to another tailnet device)"
>@echo "  │  make taildrop-file FILE=<path> [TAILDROP_HOST=server]      │send one file to the device's inbox"
>@echo "  └  make taildrop-folder DIR=<folder> [TAILDROP_HOST=server]  │send a folder (-r)"
>@echo ""
>@echo "  GitHub Actions"
...
>@echo "  │  make dok-recreate TARGET=<ctn> │force-recreate one container   │<ctn> ∈ CONTAINERS, or -all; -<ctn> is an alias"
>@echo "  │  make dok-restart TARGET=<ctn>  │restart one container          │"
>@echo "  │  make dok-stop TARGET=<ctn>     │stop one container             │"
>@echo "  │  make dok-logs TARGET=<ctn>     │tail one container's logs       │-all = follow every container"
>@echo "  └  make dok-recreate-nextcloud-db │recreate the PostgreSQL unit (docker-compose.db.yml, not in -all loops)"
>@echo ""
>@echo "  Tmux Sessions"
>@echo "  │  make tmux-new TAG=    │create detached target  │TAG= required session tag"
...
>@echo ""
>@echo "  Mailserver Registry"
>@echo "  │  make mail-gen [MAIL=] [PWD=] [QUOTA=] / mail name 7-letters auto gen if field empy, 16-char password auto gen if field empty, default quota if field empty"
>@echo "  │  make mail-gen-alias TO= / alias name 7-digits auto gen, same domain to domain alias forbidden"
>@echo "  │  make mail-del MAIL= / delete an address + all its mailbox data"
>@echo "  │  make mail-del-alias FROM= TO= / self-explanatory"
...
>@echo "  │  make nc-default-user-quota VALUE=…   default quota for NEW users (files app default_quota; syncs the recovery manifest)"
